...
try {
    ...
    if (($user_role === 'buyer' && $user_id != $transaction_data['buyer_id']) ||
        ...
        throw new Exception("您無權完成此交易。");
    }

    // 只有 'pending' 狀態的交易才能被完成
    if ($transaction_data['status'] !== 'pending') {
        throw new Exception("此交易狀態為 '{$transaction_data['status']}'，無法完成。");
    }

    // 2. 更新 transactions 表的狀態為 'completed'
    $stmt_update_transaction = $conn->prepare("UPDATE transactions SET status = 'completed' WHERE transaction_id = ?");
    ...
    // 3. 更新 book 表的 Transaction_status 為 '已售出' (下架)
    $stmt_update_book = $conn->prepare("UPDATE book SET Transaction_status = '已售出' WHERE Book_ID = ?");
    if (!$stmt_update_book) {
